...
class Window(QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.setWindowIcon(QIcon("magnum.png"))
        widget = QWidget()
    ...
    def connect(self):
        if self.lineEdit.text():
            ...
            self.label2.setText("Enter IP address")
            return
        print("Not returning")
        try:
            ...
            self.label2.setText("Connection Error")
            self.lineEdit.clear()
            self.lineEdit.setPlaceholderText("192.168.168.168")
    ...
    def send_to_rpa(self):
        ...
        while True:
            try:
                ...
                if var == b'timec':
                    self.label3.setText("Time Check Pass")
                if var == b'Geofc':
                    self.label4.setText("Geofence Check Pass")
                if var == b"GPSPF" or var == b'Geofn':
                    self.label4.setText('Waiting for GPS LOCK...')
                if var == b'SpuPA':
                    self.label6.setText("Spurious Permission Artefact Check Pass")
                if var == b'Valid':
                    self.label7.setText("Permission Artefact is Valid")
                    self.label2.setText("Ready to Arm")
                    break
                if var == b'timef':
                    self.label3.setText("Time Check Fail")
                if var == b'Geoff':
                    self.label4.setText("Geofence Check Pass Fail")
                if var == b'SpuPf':
                    self.label6.setText("Spurious Permission Artefact Check Fail")
                if var == b'faill':
                    self.label7.setText("Permission Artefact is not Valid")
    ...
    def onclick(self):
        ...
        if self.filename == None or self.filename == '':
            ...
            self.label2.setText("First Connect to RFM")
        else:
            self.send_to_rpa()
...
if __name__ == "__main__":
    ...
    splash_pix = QPixmap("magnumSplash.jpg")
